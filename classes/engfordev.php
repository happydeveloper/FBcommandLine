...
class Ot_stream
{
	...
	public function getStream($startDate, $endDate)
	{
		...
		if($this->user) {
			$this->fql = $fqlmanager->loadFql("GROUPS_WALL")." WHERE source_id = 157076174344216 AND created_time < ".$endDateType->format('U')." AND created_time >= ".$startDateType->format('U')." LIMIT 50";
